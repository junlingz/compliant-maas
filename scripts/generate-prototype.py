...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}[hidden]{display:none!important}html{scroll-behavior:smooth}body{margin:0;color:var(--text);font-family:Inter,"PingFang SC","Microsoft YaHei",system-ui,sans-serif;background:radial-gradient(circle at 18% 0,#dceaff 0,transparent 36%),linear-gradient(135deg,#f7fbff,#e8f2ff 62%,#f3f8ff);min-height:100vh}
    ...
    .compliance-panel{margin-top:12px;background:#fff;border:1px solid var(--line);border-radius:8px}
    .compliance-panel>summary{display:flex;align-items:center;gap:10px;padding:12px 16px;cursor:pointer;list-style:none;color:#4f5b70}
    .compliance-panel>summary::-webkit-details-marker{display:none}.compliance-panel>summary:before{content:"›";font-size:18px;transition:transform .15s}
    .compliance-panel[open]>summary:before{transform:rotate(90deg)}
    .compliance-panel>summary b{font-size:12px}.compliance-panel>summary small{color:var(--muted);font-size:11px}
    .compliance-panel>summary .badge{margin-left:auto}
    .audit-card{border:0;border-top:1px solid var(--line);border-radius:0;padding:16px;box-shadow:none;max-height:none;overflow:visible}
    .requirements{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.req{border:1px solid var(--line);border-radius:7px;background:#fafbfc;padding:10px}
    .req-head{display:flex;gap:8px;align-items:flex-start}.check{width:18px;height:18px;border-radius:50%;background:#e7f5ef;color:var(--green);display:grid;place-items:center;flex:none;font-size:10px}
    .req strong{font-size:11px}.req .clause{display:block;color:var(--muted);font-size:10px}.req-items{display:flex;gap:5px;flex-wrap:wrap;margin:8px 0 0 26px}
    .req-items button{border:1px solid var(--line);background:#fff;border-radius:5px;color:#5f6b7e;padding:4px 6px;font-size:10px}
    ...
    @media(max-width:760px){
      ...
      .requirements{grid-template-columns:1fr}.compliance-panel>summary{padding:11px 12px}.compliance-panel>summary small{display:none}
...
<body>
  ...
  const navItems = [{id:'overview',name:'平台总览'},...modules.map(m=>({id:m.id,name:m.name,count:m.sections.length})),{id:'tasks',name:'统一任务中心'},{id:'docs',name:'技术文档中心'},{id:'coverage',name:'合规覆盖中心'}];
  const modelNames = {
  ...
  function renderNav(){
    const groups=[
      ...
      {label:'服务与治理',items:navItems.filter(x=>['inference','docs','coverage'].includes(x.id))}
    ];
  ...
  function requirementsPanel(section,module){
    ...
    return `<details class="compliance-panel"><summary><b>合规与需求映射</b><small>${section.clause} · ${count} 个核验点，按需展开</small><span class="badge green">覆盖完整</span></summary><aside class="audit-card"><div class="requirements">${section.features.map((f,fi)=>`<article class="req" data-clause="${f.clause}"><div class="req-head"><span class="check">✓</span><div><strong>${f.name}</strong><span class="clause">${f.clause}</span></div><button class="btn sm" style="margin-left:auto" data-requirement="${module.id}:${state.section[module.id]||0}:${fi}">定位控件</button></div>${f.items.length?`<div class="req-items">${f.items.map((item,ii)=>`<button data-clause="${f.clause}#${ii+1}" data-requirement="${module.id}:${state.section[module.id]||0}:${fi}:${ii}">${item.name}</button>`).join('')}</div>`:''}</article>`).join('')}</div></aside></details>`;
  }
...
validation: pending</div><div class="footer-actions"><button class="btn" data-action="saveParallel">保存可读配置</button><button class="btn primary" data-action="validateParallel">校验模型、范式与资源</button></div>`;
    ...
    return pageToolbar('生产工作台',actions)+
      ...
      `<div class="overview-grid">
        <div class="overview-primary">
          <section class="panel"><div class="panel-header"><div><h2>最近任务</h2><p>跨训练、微调、评测和部署的实时进度</p></div><button class="btn sm" data-page="tasks">查看全部</button></div><div class="task-list">${state.tasks.slice(0,4).map(task=>`<div class="task-row"><div><b>${task.name}</b><small>${task.id} · ${task.gpu}</small></div><span class="status ${task.status==='运行中'?'running':task.status==='失败'?'failed':task.status==='排队中'?'queued':''}">${task.status}</span><small>${task.time}</small><div class="task-progress"><span class="progress"><i style="width:${task.progress}%"></i></span>${task.progress}%</div></div>`).join('')}</div></section>
          <section class="panel"><div class="panel-header"><div><h2>模型生产流程</h2><p>从数据准备到生产发布，阶段状态可追溯</p></div><span class="badge green">质量门禁正常</span></div><div class="process">${processSteps.map((p,i)=>`<article class="process-step" data-process="${i}"><span class="badge">0${i+1}</span><br><b>${p.name}</b><p>${p.description.slice(0,53)}…</p></article>`).join('')}</div></section>
        </div>
        <aside class="overview-aside">
          <section class="panel"><div class="panel-header"><div><h3>快捷操作</h3><p>从常用入口开始工作</p></div></div><div class="quick-actions"><button class="quick-action" data-page="autoregressive"><b>创建预训练</b><span>模型、数据与资源配置</span></button><button class="quick-action" data-page="evaluation"><b>发起模型评测</b><span>基准、指标与报告</span></button><button class="quick-action" data-page="inference"><b>发布推理服务</b><span>版本、网关与流量</span></button><button class="quick-action" data-page="coverage"><b>查看合规状态</b><span>${totalFeatures()} 项能力已映射</span></button></div></section>
          <section class="panel"><div class="panel-header"><div><h3>资源与服务健康</h3><p>过去 15 分钟</p></div><button class="btn sm" data-action="notifications">详情</button></div><div class="health-list"><div class="health-row"><span class="health-dot"></span><div><b>训练集群</b><small>32 个节点在线</small></div><span class="value">正常</span></div><div class="health-row"><span class="health-dot warn"></span><div><b>GPU 资源池</b><small>2 个任务等待调度</small></div><span class="value">84%</span></div><div class="health-row"><span class="health-dot"></span><div><b>推理网关</b><small>P95 186ms</small></div><span class="value">99.97%</span></div><div class="health-row"><span class="health-dot danger"></span><div><b>异常任务</b><small>RDMA 通信超时</small></div><span class="value">1</span></div></div></section>
        </aside>
      </div>
      <section style="margin-top:18px"><div class="panel-header"><div><h2>能力工作台</h2><p>按模型生产任务进入对应工作区</p></div></div><div class="grid overview-modules">${modules.map((m,i)=>`<article class="card module-card" data-page="${m.id}"><span class="index">工作台 0${i+1}</span><h3>${m.name}</h3><p>${m.description.slice(0,83)}…</p><div class="chips"><span class="chip">${m.sections.length} 个能力组</span><span class="chip">${m.sections.reduce((s,x)=>s+x.features.length,0)} 项功能</span></div></article>`).join('')}</div></section>`;
  }
  function tasksPage(){
  ...
  function coveragePage(){
    return pageToolbar('合规覆盖中心','<button class="btn" data-action="exportMatrix">导出覆盖矩阵</button><button class="btn primary" data-action="runSelfCheck">运行完整性自检</button>')+
